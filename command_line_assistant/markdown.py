...
This module provides a comprehensive set of extensions for python-markdown that render
markdown elements using ANSI escape codes suitable for terminal display, rather than HTML.

The extensions leverage the existing ColorDecorator system for consistent color handling
and provide terminal-friendly formatting for all common markdown elements.
"""
...
class ANSIRenderer:
    ...
    def code_block(self, text: str, language: str = "") -> str:
        ...
        lines = [f"{BRIGHT_CYAN}{line}{RESET}" for line in text.rstrip().split("\n")]
        longest_line_length = max(len(line) for line in text.rstrip().split("\n"))
        padding = longest_line_length - len(lang_text) + 6
        header = f"{BRIGHT_RED}──{lang_text}{'─' * padding}{RESET}"
        footer = f"{BRIGHT_RED}{'─' * (padding + len(lang_text) + 2)}{RESET}"
        return f"\n{header}\n" + "\n".join(lines) + f"\n{footer}\n"

    def header(self, text: str, level: int) -> str:
        """Format header."""
        colors = [BRIGHT_GREEN, GREEN, BRIGHT_YELLOW, YELLOW, BRIGHT_MAGENTA, MAGENTA]
        color = colors[min(level - 1, 5)]

        if level == 1:
            return f"\n{color}{text.upper()}{RESET}\n{color}{'═' * len(text)}{RESET}\n"
        elif level == 2:
            return f"\n{color}{text}{RESET}\n{color}{'─' * len(text)}{RESET}\n"
        else:
            prefix = "#" * level
    ...
    def link(self, text: str, url: str, title: str = "") -> str:
        """Format link."""
        link_text = f"{BRIGHT_BLUE}{text}{RESET}"
        if title:
            return f"{link_text} ({BRIGHT_BLUE}{url}{RESET}, {title})"
        return f"{link_text} ({BRIGHT_BLUE}{url}{RESET})"

    def image(self, alt_text: str, url: str, title: str = "") -> str:
        """Format image (as text representation)."""
        return f"[Image: {alt_text}] ({BRIGHT_BLUE}{url}{RESET})"

    def blockquote(self, text: str) -> str:
        """Format blockquote."""
        lines = text.strip().split("\n")
        quoted_lines = [f"{BRIGHT_BLACK}│{RESET} {line}" for line in lines]
        return (
            f"\n{BRIGHT_BLACK}┌─{RESET}\n"
            + "\n".join(quoted_lines)
            + f"\n{BRIGHT_BLACK}└─{RESET}\n"
    ...
    def list_item(self, text: str, ordered: bool = False, index: int = 1) -> str:
        """Format list item."""
        if ordered:
            marker = f"{BRIGHT_CYAN}{index}.{RESET}"
        else:
            marker = f"{BRIGHT_CYAN}•{RESET}"
    ...
    def format_table(self, rows: List[List[str]], header_row: bool = True) -> str:
        ...
        for row_idx, row in enumerate(rows):
            ...
            for col, cell in enumerate(row):
                if col < len(col_widths):
                    ...
                    if row_idx == 0 and header_row:
                        # Header cell
                        formatted_cell = f"{' ' * left_pad}{BRIGHT_GREEN}{cell}{RESET}{' ' * right_pad}"
                    else:
                        # Regular cell
            ...
            # Row content
            row_content = f"{BRIGHT_BLACK}│{RESET}"
            row_content += f"{BRIGHT_BLACK}│{RESET}".join(formatted_cells)
            row_content += f"{BRIGHT_BLACK}│{RESET}"
            result.append(row_content)

            # Add separator after header
            if row_idx == 0 and header_row and len(rows) > 1:
                sep_border = f"{BRIGHT_BLACK}├"
                for i, width in enumerate(col_widths):
                    sep_border += "─" * width
                    if i < len(col_widths) - 1:
                        sep_border += "┼"
...
class ANSITreeProcessor(Treeprocessor):
    ...
    def _set_parent_relationships(
        self, elem: etree.Element, parent: etree.Element = None
    ) -> None:
    ...
    def _format_by_tag(self, tag: str, elem: etree.Element, content: str) -> str:
        ...
        if tag in ("strong", "b"):
            return self.renderer.bold(content)
        elif tag in ("em", "i"):
            return self.renderer.italic(content)
        elif tag == "u":
            return self.renderer.underline(content)
        elif tag in ("del", "s"):
            return self.renderer.strikethrough(content)
        elif tag == "code":
        ...
        # Block elements
            ...
            return self._format_code_block(elem, content)
        elif self._is_header_tag(tag):
            level = int(tag[1])
            return self.renderer.header(content, level)
        elif tag == "blockquote":
            return self.renderer.blockquote(content)
        elif tag == "hr":
        ...
        # Links and media
            ...
            return self._format_link(elem, content)
        elif tag == "img":
            return self._format_image(elem)

        # Lists and tables
        elif tag == "li":
            return self._format_list_item(elem, content)
        elif tag in ("ul", "ol"):
            return content
        elif tag == "table":
...
class ANSIExtension(Extension):
    """Main extension that provides ANSI terminal output instead of HTML."""
...
def makeExtension(**kwargs) -> ANSIExtension:
    """Factory function for creating the extension."""
    return ANSIExtension(**kwargs)
...
def markdown_to_ansi(text: str, **kwargs) -> str:
    ...
    # Ensure our extension and tables extension are included
    extensions = kwargs.get("extensions", [])
    if "command_line_assistant.markdown" not in extensions:
        extensions.append("command_line_assistant.markdown")
...
def create_ansi_markdown_processor(**renderer_kwargs) -> markdown.Markdown:
    """Create a markdown processor configured for ANSI output.

    Args:
        **renderer_kwargs: Arguments passed to ANSIRenderer constructor

    Returns:
        Configured markdown processor
    """

    renderer = ANSIRenderer(**renderer_kwargs) if renderer_kwargs else ANSIRenderer()
    extension = ANSIExtension(renderer=renderer)

    return markdown.Markdown(extensions=[extension, "tables"])
